...
que YOLOv2 es capaz de superar a SSD512 obteniendo un mejor \acrfullr{map}(parámetro de caracterización del error medio típico sobre todo el dataset) con el doble de rendimiento.
...
Podemos observar además, que YOLOv2 tiene ciertos problemas al tratar con objetos pequeños. Algunos estudios\cite{bhagyaOverviewDeepLearning2019} ya han observado que es un problema bastante típico de YOLO y es una de las situaciones en las que 
peor funciona.\newline
...
\subsection{Entornos de desarrollo y despliegue en \textit{HardWare} transparente}

\begin{itemize}
    \item \textbf{Entornos de desarrollo de Redes Neuronales}: Para trabajar en la creación de redes neuronales, se usan principalmente dos librerías: pytorch y Keras. Siendo una de las principales 
    diferencias bla bla bla. En el caso de YOLO, \todo{Metemos su historia}, por lo tanto no es necesario saber de las anteriores herramientas mencionadas.
    \item \textbf{Entornos para uso de análisis de flujo óptico}: En la mayoría de lenguajes existen librerías que implementan los algoritmos de análisis de flujo óptico, pero, al ser herramientas de 
    caracterización de imagen, existen muchas facilidades en entornos matemáticos como MATLAB o de procesado de datos como el lenguaje de programación Python, mas concretamente la libreria de \textit{OpenCV}.
    
    \item \textbf{Despliegue de redes transparentes al \textit{HardWare}}
\end{itemize}
