...
Lo primero que se realizó fue un pequeño aumento de datos en los que se aumentó el conjunto de datos a 40 imágenes. Como se buscaba que la red generalizase mejor el concepto de trucha, se añadieron imágenes sin truchas como las de 
...
\subsubsection{Pruebas de desarrollo: sistema secuencial con \texttt{YOLO}}
...
\begin{figure}[H]
    ...
    \caption{Diagrama del primer sistema \texttt{StandAlone} a desarrollar}
    \label{fig:IdeaSistemaBasico}
\end{figure}

Para plantear el procesamiento, primero se deben entender que datos es capaz de devolver un modelo \texttt{YOLO}, su formato y seleccionar los mas relevantes.\newline
En los modelos de detección de objetos, hay 2 modelos principales que se quieren tratar en este trabajo:

\begin{itemize}
    \item Detección tradicional con \texttt{Bounding Boxes} con ejes paralelos a los ejes de la imagen.
    \item Detección con \texttt{Bounding Boxes} orientadas, que es capaz de minimizar el área que define al pez, pero tarda más en realizar el procesamiento y tiene resultados en distinto formato.
\end{itemize}

\subsubsection{Obtención del número de truchas}
...
misma posición de forma superpuesta. Un ejemplo de esto se ve en la \autoref{fig:Superposicion}. En este caso es esperable obtener 1 datos por fotograma, por lo tanto es necesario decidir la mejor \texttt{Bounding Box}.
